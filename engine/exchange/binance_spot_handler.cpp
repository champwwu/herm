...
BinanceSpotHandler::BinanceSpotHandler(const engine::common::ConfigManager& config)
...
BinanceSpotHandler::~BinanceSpotHandler() {
  // Disconnect() should have been called explicitly before destruction
  // Calling it here as safety fallback
  if (shared_connection_) {
    SPDLOG_WARN("BinanceSpotHandler: Connection not empty in destructor, calling Disconnect()");
    Disconnect();
  }
}

void BinanceSpotHandler::Initialize(const engine::common::ConfigManager& config) {
  config_ = &config;
  
  // Parse binance config section from market_data.binance
  auto binance_node = config.GetNodeValue("market_data.binance");
  if (binance_node.is_object() && binance_node.contains("spot")) {
    auto spot_node = binance_node["spot"];
    
    if (spot_node.contains("websocket_url_template") && spot_node["websocket_url_template"].is_string()) {
      websocket_url_template_ = spot_node["websocket_url_template"].get<std::string>();
    } else {
      // Default template
      websocket_url_template_ = "wss://stream.binance.com:9443/ws/{symbol}@depth@100ms";
    }
    
    if (spot_node.contains("rest_url") && spot_node["rest_url"].is_string()) {
      rest_url_ = spot_node["rest_url"].get<std::string>();
      // Set the base URL on the HTTP client
      http_client_->SetBaseUrl(rest_url_);
    } else {
      rest_url_ = "https://api.binance.com";
    }
    
    if (spot_node.contains("rate_limit") && spot_node["rate_limit"].is_object()) {
      auto rate_limit_node = spot_node["rate_limit"];
  ...
  SPDLOG_DEBUG("BinanceSpotHandler initialized - websocket_template: {}, rest_url: {}, rate_limit: {} req/s",
              websocket_url_template_, rest_url_, rate_limit_rps_);
}

bool BinanceSpotHandler::Subscribe(const engine::common::Instrument& instrument) {
  if (instrument.instrument_type != engine::common::InstrumentType::SPOT) {
    SPDLOG_WARN("BinanceSpotHandler: Cannot subscribe non-SPOT instrument: {}", instrument.instrument_id);
    return false;
  }
  
  {
    ...
    if (subscribed_instruments_.find(instrument.instrument_id) != subscribed_instruments_.end()) {
      SPDLOG_WARN("BinanceSpotHandler: Already subscribed to instrument: {}", instrument.instrument_id);
      return false;
    }
  ...
  SPDLOG_DEBUG("BinanceSpotHandler: Subscribed to instrument: {}", instrument.instrument_id);
  
  // Subscribe on shared connection (will reconnect if needed)
  return SubscribeInstrument(instrument);
}

bool BinanceSpotHandler::Unsubscribe(const std::string& instrument_id) {
  ...
  
  {
    ...
    if (it == subscribed_instruments_.end()) {
      SPDLOG_WARN("BinanceSpotHandler: Not subscribed to instrument: {}", instrument_id);
      return false;
    }
  ...
  SPDLOG_DEBUG("BinanceSpotHandler: Unsubscribed from instrument: {}", instrument_id);
  return true;
}

bool BinanceSpotHandler::Connect() {
  return EnsureConnected();
}

void BinanceSpotHandler::Disconnect() {
...
bool BinanceSpotHandler::IsConnected() const {
  std::lock_guard<std::mutex> lock(connection_mutex_);
  return shared_connection_ && shared_connection_->connected_.load();
}

SharedConnection* BinanceSpotHandler::GetOrCreateSharedConnection() {
  std::lock_guard<std::mutex> lock(connection_mutex_);
  
  if (!shared_connection_) {
    shared_connection_ = std::make_unique<SharedConnection>();
    shared_connection_->instrument_type_ = engine::common::InstrumentType::SPOT;
...
bool BinanceSpotHandler::EnsureConnected() {
...
void BinanceSpotHandler::DisconnectSharedConnection() {
  std::lock_guard<std::mutex> lock(connection_mutex_);
  ...
  if (shared_connection_ && shared_connection_->running_.exchange(false)) {
    ...
    SPDLOG_DEBUG("BinanceSpotHandler: Shared connection disconnected");
  }
  
  shared_connection_.reset();
}

std::string BinanceSpotHandler::GetBinanceSymbol(const engine::common::Instrument& instrument) const {
  std::string symbol = instrument.GetSymbol();  // Returns "BTCUSDT"
  std::transform(symbol.begin(), symbol.end(), symbol.begin(), ::tolower);
  return symbol;  // Returns "btcusdt"
}

std::string BinanceSpotHandler::GetStreamName(const engine::common::Instrument& instrument) const {
  // Binance stream format: {symbol}@depth (e.g., "btcusdt@depth")
  return GetBinanceSymbol(instrument) + "@depth";
}

InstrumentOrderBookState* BinanceSpotHandler::FindOrderBookByStream(const std::string& stream) {
...
std::string BinanceSpotHandler::GetInstrumentIdFromStream(const std::string& stream) const {
  std::lock_guard<std::mutex> lock(stream_mapping_mutex_);
  auto it = stream_to_instrument_id_.find(stream);
  if (it != stream_to_instrument_id_.end()) {
    return it->second;
...
std::string BinanceSpotHandler::BuildCombinedStreamUrl(const std::vector<const engine::common::Instrument*>& instruments) const {
  if (instruments.empty()) {
    return "";
  }
...
bool BinanceSpotHandler::SubscribeInstrument(const engine::common::Instrument& instrument) {
...
void BinanceSpotHandler::UnsubscribeInstrument(const std::string& instrument_id) {
...
bool BinanceSpotHandler::ConnectSharedConnection(const std::vector<const engine::common::Instrument*>& instruments) {
  auto* conn = GetOrCreateSharedConnection();
  if (!conn) {
    return false;
  }
  ...
  try {
    ...
    std::string default_host = "stream.binance.com";
    std::string default_port = "9443";
    ...
    SPDLOG_DEBUG("BinanceSpotHandler: Connecting shared connection to {}:{}{}",
    ...
    conn->resolver_->async_resolve(conn->host_, conn->port_,
        [this, conn](beast::error_code ec, tcp::resolver::results_type results) {
          if (ec) {
            SPDLOG_ERROR("BinanceSpotHandler shared connection resolve error: {}", ec.message());
            conn->running_ = false;
            if (conn->ioc_) {
              conn->ioc_->stop();
            }
          ...
          beast::get_lowest_layer(*conn->ws_).async_connect(results,
              [this, conn](beast::error_code ec, tcp::resolver::results_type::endpoint_type) {
                if (ec) {
                  SPDLOG_ERROR("BinanceSpotHandler shared connection connect error: {}", ec.message());
                  conn->running_ = false;
                  if (conn->ioc_) {
                    conn->ioc_->stop();
                  }
                ...
                conn->ws_->next_layer().async_handshake(ssl::stream_base::client,
                    [this, conn](beast::error_code ec) {
                      if (ec) {
                        SPDLOG_ERROR("BinanceSpotHandler shared connection SSL handshake error: {}", ec.message());
                        conn->running_ = false;
                        if (conn->ioc_) {
                          conn->ioc_->stop();
                        }
    ...
    conn->running_ = true;
    conn->io_thread_ = std::thread(&BinanceSpotHandler::RunIO, this, conn);
    ...
    SPDLOG_ERROR("BinanceSpotHandler shared connection error: {}", e.what());
    return false;
  }
}

bool BinanceSpotHandler::ConnectSharedConnectionWithRetry(const std::vector<const engine::common::Instrument*>& instruments, int max_retries) {
  for (int attempt = 0; attempt < max_retries; ++attempt) {
    if (attempt > 0) {
      // Exponential backoff: 1s, 2s, 4s
      int delay_ms = (1 << (attempt - 1)) * 1000;
      SPDLOG_INFO("BinanceSpotHandler: Retry attempt {}/{} for shared connection after {}ms",
    ...
    try {
      if (ConnectSharedConnection(instruments)) {
        SPDLOG_INFO("BinanceSpotHandler: Successfully connected shared connection on attempt {}/{}",
                    attempt + 1, max_retries);
        return true;
      }
    } catch (const std::exception& e) {
      SPDLOG_WARN("BinanceSpotHandler: Shared connection attempt {}/{} failed: {}",
                  attempt + 1, max_retries, e.what());
    }
  }
  
  SPDLOG_ERROR("BinanceSpotHandler: Failed to connect shared connection after {} attempts", max_retries);
  return false;
}

void BinanceSpotHandler::RunIO(SharedConnection* conn) {
  // Pin to CPU core if manager is set
  if (cpu_pinning_manager_) {
    try {
      cpu_pinning_manager_->PinCurrentThread("binance_spot_io");
    } catch (const std::exception& e) {
      SPDLOG_ERROR("Failed to pin binance_spot_io thread: {}", e.what());
      conn->running_ = false;
      if (conn->ioc_) {
        conn->ioc_->stop();
      }
...
void BinanceSpotHandler::OnHandshake(SharedConnection* conn, beast::error_code ec) {
  if (ec) {
    SPDLOG_ERROR("BinanceSpotHandler: Shared connection handshake error: {}", ec.message());
    return;
  }

  conn->connected_ = true;
  SPDLOG_DEBUG("BinanceSpotHandler: Shared connection established");
  ...
  if (instruments.empty()) {
    SPDLOG_WARN("BinanceSpotHandler: No instruments to initialize on shared connection");
    DoRead(conn);
    return;
  }
...
void BinanceSpotHandler::DoRead(SharedConnection* conn) {
...
void BinanceSpotHandler::OnRead(SharedConnection* conn, beast::error_code ec, std::size_t bytes_transferred, beast::flat_buffer buffer) {
  ...
  if (ec == websocket::error::closed) {
    SPDLOG_DEBUG("BinanceSpotHandler: Shared connection WebSocket closed");
  ...
  if (ec) {
    SPDLOG_ERROR("BinanceSpotHandler: Shared connection read error: {}", ec.message());
    // Trigger reconnection
    Reconnect(conn);
    return;
  }
  ...
  SPDLOG_TRACE("BinanceSpotHandler: Received message ({} bytes)", bytes_transferred);
  ...
  try {
    ...
    if (json.contains("stream") && json.contains("data")) {
      ...
      if (!ob_state) {
        SPDLOG_WARN("BinanceSpotHandler: Received message for unknown stream: {}", stream_name);
        DoRead(conn);
        return;
      }
      ...
      if (data.contains("e") && data["e"] == "depthUpdate") {
        if (!ob_state->snapshot_received_) {
          ...
          SPDLOG_TRACE("BinanceSpotHandler: Buffering event (snapshot not received yet) for {}", ob_state->instrument_id);
        } else {
          // Apply update directly
          SPDLOG_TRACE("BinanceSpotHandler: Processing depthUpdate event for {}", ob_state->instrument_id);
          ApplyUpdate(ob_state, data);
        }
      } else {
        SPDLOG_DEBUG("BinanceSpotHandler: Received non-depthUpdate message for {}: {}", ob_state->instrument_id, message.substr(0, 100));
      }
    } else {
      SPDLOG_DEBUG("BinanceSpotHandler: Received non-stream message: {}", message.substr(0, 100));
    }
  } catch (const nlohmann::json::exception& e) {
    SPDLOG_WARN("BinanceSpotHandler: JSON parse error: {}", e.what());
  } catch (const std::exception& e) {
    SPDLOG_WARN("BinanceSpotHandler: Parse error: {}", e.what());
  }

  DoRead(conn);
}

void BinanceSpotHandler::HandlePing(SharedConnection* conn, const std::string& payload) {
  ...
  if (ec) {
    SPDLOG_WARN("BinanceSpotHandler: Error sending pong: {}", ec.message());
  } else {
    conn->last_ping_time_ = std::chrono::steady_clock::now();
    SPDLOG_TRACE("BinanceSpotHandler: Sent pong");
  }
}

void BinanceSpotHandler::GetSnapshot(const engine::common::Instrument& instrument) {
  SPDLOG_DEBUG("BinanceSpotHandler: GetSnapshot called for {}", instrument.instrument_id);
  ...
  std::thread([this, instrument_id, symbol]() {
    SPDLOG_TRACE("BinanceSpotHandler: GetSnapshot thread started for {}", instrument_id);
    ...
    SPDLOG_DEBUG("BinanceSpotHandler: Fetching snapshot for symbol {} ({})", symbol, instrument_id);
    auto snapshot = http_client_->GetDepthSnapshot(symbol, false);  // false = spot

    if (snapshot.lastUpdateId == 0 || snapshot.bids.empty() || snapshot.asks.empty()) {
      SPDLOG_ERROR("BinanceSpotHandler: Failed to get snapshot for {}", instrument_id);
    ...
    SPDLOG_DEBUG("BinanceSpotHandler: Snapshot retrieved, calling ApplySnapshot for {}", instrument_id);
    ApplySnapshot(instrument_id, snapshot);
    SPDLOG_TRACE("BinanceSpotHandler: ApplySnapshot completed for {}", instrument_id);
  }).detach();
}

void BinanceSpotHandler::ApplySnapshot(const std::string& instrument_id, const BinanceHttpClient::DepthSnapshot& snapshot) {
  SPDLOG_DEBUG("BinanceSpotHandler: ApplySnapshot started for {}", instrument_id);
  ...
  InstrumentOrderBookState* ob_state = nullptr;
  {
    ...
    auto it = order_books_.find(instrument_id);
    if (it == order_books_.end()) {
      SPDLOG_ERROR("BinanceSpotHandler: Order book state not found for {}", instrument_id);
      return;
    }
    ob_state = &it->second;
  }
  
  {
    ...
    // Apply snapshot
    SPDLOG_DEBUG("BinanceSpotHandler: Applying {} bids and {} asks from snapshot", snapshot.bids.size(), snapshot.asks.size());
    ...
    SPDLOG_DEBUG("BinanceSpotHandler: Snapshot applied for {}, lastUpdateId: {}, bids: {}, asks: {}",
    ...
    SPDLOG_TRACE("BinanceSpotHandler: Applying {} buffered events", ob_state->buffered_events_.size());
  ...
  if (instrument) {
    SPDLOG_TRACE("BinanceSpotHandler: Calling NotifyUpdate with {} bids, {} asks", ob_state->temp_bids_.size(), ob_state->temp_asks_.size());
    NotifyUpdate(*instrument, ob_state->temp_bids_, ob_state->temp_asks_);
    SPDLOG_TRACE("BinanceSpotHandler: NotifyUpdate completed, ApplySnapshot finished");
  } else {
    SPDLOG_ERROR("BinanceSpotHandler: Instrument {} not found for callback", instrument_id);
  }
}

void BinanceSpotHandler::ApplyUpdate(InstrumentOrderBookState* ob_state, const nlohmann::json& event) {
  {
    std::lock_guard<std::mutex> lock(ob_state->order_book_mutex_);
    ...
    // Validate update IDs
    if (!ValidateUpdateId(ob_state, first_update_id, final_update_id)) {
      SPDLOG_WARN("BinanceSpotHandler: Update ID validation failed for {}, reinitializing order book", ob_state->instrument_id);
  ...
  const engine::common::Instrument* instrument = FindInstrument(ob_state->instrument_id);
  if (instrument) {
    SPDLOG_DEBUG("BinanceSpotHandler: Sending update to callback - {} bids, {} asks", ob_state->temp_bids_.size(), ob_state->temp_asks_.size());
    NotifyUpdate(*instrument, ob_state->temp_bids_, ob_state->temp_asks_);
  } else {
    SPDLOG_ERROR("BinanceSpotHandler: Instrument {} not found for callback", ob_state->instrument_id);
  }
}

bool BinanceSpotHandler::ValidateUpdateId(InstrumentOrderBookState* ob_state, int64_t first_id, int64_t final_id) {
  // Check: u (final update ID) >= local order book update ID
  if (final_id < ob_state->last_update_id_) {
    SPDLOG_DEBUG("BinanceSpotHandler: Event too old (u={} < last_update_id={}) for {}", 
                 final_id, ob_state->last_update_id_, ob_state->instrument_id);
    return false;  // Discard old event
  }

  // Check: U (first update ID) <= local order book update ID + 1
  if (first_id > ob_state->last_update_id_ + 1) {
    SPDLOG_WARN("BinanceSpotHandler: Gap detected (U={} > last_update_id+1={}) for {}", 
                first_id, ob_state->last_update_id_ + 1, ob_state->instrument_id);
...
void BinanceSpotHandler::FilterBufferedEvents(InstrumentOrderBookState* ob_state) {
  // Filter buffered events: only apply events that are newer than or equal to snapshot
  // According to Binance docs: final_update_id >= snapshot.lastUpdateId
  ...
  for (const auto& event : ob_state->buffered_events_) {
    ...
    // Only apply events where final_update_id >= snapshot.lastUpdateId
    // This ensures we don't apply stale events that arrived before the snapshot
    if (final_update_id < ob_state->last_update_id_) {
      SPDLOG_DEBUG("BinanceSpotHandler: Discarding buffered event (too old): final_id={} < snapshot_id={}", 
                   final_update_id, ob_state->last_update_id_);
      discarded_count++;
      continue;
    }

    // Check for gaps - if found, stop processing buffered events
    if (first_update_id > ob_state->last_update_id_ + 1) {
      SPDLOG_DEBUG("BinanceSpotHandler: Gap detected in buffered events (first_id={} > last_id+1={}), stopping buffer processing", 
                   first_update_id, ob_state->last_update_id_ + 1);
      gap_found = true;
      break;  // Stop processing buffered events on gap
    }
  ...
  if (gap_found) {
    SPDLOG_DEBUG("BinanceSpotHandler: Applied {}/{} buffered events ({} discarded as stale, stopped due to gap)", 
                ob_state->temp_valid_events_.size(), ob_state->buffered_events_.size(), discarded_count);
  } else if (discarded_count > 0) {
    SPDLOG_DEBUG("BinanceSpotHandler: Applied {}/{} buffered events ({} discarded as stale)", 
                ob_state->temp_valid_events_.size(), ob_state->buffered_events_.size(), discarded_count);
  }
}

void BinanceSpotHandler::ApplyBufferedEventsToOrderBook(InstrumentOrderBookState* ob_state) {
  // Apply valid events
...
void BinanceSpotHandler::PrepareCallbackData(InstrumentOrderBookState* ob_state) {
  // Copy order book data for callback (while still holding lock)
  // Reuse temp_bids_/temp_asks_ buffers
  ob_state->temp_bids_.clear();
  ob_state->temp_asks_.clear();
  ...
  SPDLOG_TRACE("BinanceSpotHandler: Preparing to send initial order book to callback");
  for (const auto& [price, qty] : ob_state->local_order_book_bids_) {
    ob_state->temp_bids_.emplace_back(price, qty);
  }
  for (const auto& [price, qty] : ob_state->local_order_book_asks_) {
...
void BinanceSpotHandler::Reconnect(SharedConnection* conn) {
  if (!conn->running_.load()) {
    return;
  }
  ...
  SPDLOG_DEBUG("BinanceSpotHandler: Scheduling shared connection reconnection attempt {} in {} seconds",
  ...
  std::thread([this, conn]() {
    ...
    if (conn->running_.load() && !conn->connected_.load()) {
      SPDLOG_DEBUG("BinanceSpotHandler: Reconnecting shared connection");
      DisconnectSharedConnection();
...
bool BinanceSpotHandler::ParseMessage(const std::string& message,
  ...
  try {
    ...
    SPDLOG_ERROR("BinanceSpotHandler: Failed to parse message: {}", e.what());
  }
  return false;
}

const engine::common::Instrument* BinanceSpotHandler::FindInstrument(const std::string& instrument_id) const {
  std::lock_guard<std::mutex> lock(subscribed_mutex_);
  auto it = subscribed_instruments_.find(instrument_id);
  if (it != subscribed_instruments_.end()) {
    return it->second;
