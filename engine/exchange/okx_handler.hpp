...
class OKXHandler : public MarketDataHandler {
 public:
  OKXHandler(const engine::common::ConfigManager& config);
  ~OKXHandler() override;
 ...
 private:
  ...
  std::unique_ptr<OKXHttpClient> http_client_;
  ...
  void ApplySnapshot(const std::string& instrument_id, const OKXHttpClient::DepthSnapshot& snapshot);
  void ApplyUpdate(InstrumentOrderBookState* order_book_state, const nlohmann::json& event);
  bool ValidateUpdateId(InstrumentOrderBookState* order_book_state, int64_t update_id);
  void ReconnectAll();
  
  // Symbol conversion and routing
  std::string GetOKXSymbol(const engine::common::Instrument& instrument) const;
