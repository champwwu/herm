...
class BinanceFuturesHandler : public MarketDataHandler {
 public:
  BinanceFuturesHandler(const engine::common::ConfigManager& config);
  ~BinanceFuturesHandler() override;
 ...
 private:
  ...
  // Stream to instrument ID mapping
  ...
  bool ValidateUpdateId(InstrumentOrderBookState* order_book_state, int64_t first_id, int64_t final_id, int64_t prev_id);
